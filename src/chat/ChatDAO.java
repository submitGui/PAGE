...
public class ChatDAO {
	...
	public ArrayList<ChatDTO> getChatListByID(String fromID, String toID, String chatID ) {
		...
		try {
			...
			pstmt.setString(3, toID);// 자신이 받던 주던 순서를 맞춰주기위한것
	...
	public ArrayList<ChatDTO> getChatListByRecent(String fromID, String toID, int number ) {
		...
		try {
			...
			pstmt.setString(8, toID);// 자신이 받던 주던 순서를 맞춰주기위한것
	...
	public int readChat(String fromID, String toID) {
		...
		try {
			...
			return pstmt.executeUpdate(); //실행된 결과를 반환
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			try {
	...
	public int getAllUnreadChat(String userID) {
		...
		try {
			...
			if(rs.next()) {
				return rs.getInt("COUNT(chatID)");//결과를 가져옴
			}
			return 0; //받은 메시지가없다라는 뜻
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			try {
	...
	//최근메시지 함을 가져오는것
	public ArrayList<ChatDTO> getBox(String userID) { //메세지함
		ArrayList<ChatDTO> chatList = null;
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	//특정한 사용자와 대화를 한 내용중에서 얼마나 안읽었는지를 알려주는 함수  
	public int getUnreadChat(String fromID, String toID) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String SQL="SELECT COUNT(chatID) FROM CHAT WHERE fromID = ? AND toID = ? AND chatRead = 0";//어떤사용자로부터 도착한 메세지인지 확인하는 쿼리문 
		try {
			conn = dataSource.getConnection();
			pstmt = conn.prepareStatement(SQL);
			pstmt.setString(1, fromID);
